...
#End Region
    ...
    Public Overloads Sub Dispose()
        MyBase.Dispose()
        Me.ClearItems()
        RemoveHandler JobLimitsProvider.GotRefreshed, AddressOf Me.GotRefreshed
...
#Region "Update methods & callbacks"
    ...
    Private Delegate Sub degGotNewItems(ByVal news As List(Of String), ByVal newItems As Dictionary(Of String, jobLimitInfos))
    Private Sub GotNewItems(ByVal news As List(Of String), ByVal newItems As Dictionary(Of String, jobLimitInfos))
        ...
        If _hasToLock Then
            Me.BeginUpdate()
        End If
        For Each jobName As String In news
            If _dico.ContainsKey(jobName) = False Then
                Dim job As New cJobLimit(newItems(jobName))
                job.NewCount = 3
                _dico.Add(jobName, job)
                ...
                For x As Integer = 1 To _subItems.Length - 1
                    _subItems(x) = ""
                Next
                AddItemWithStyle(jobName, job).SubItems.AddRange(_subItems)
            End If
        Next
    ...
    Private Delegate Sub degGotRefreshed(ByVal _dicoNew As List(Of String), ByVal _dicoDel As List(Of String), ByVal Dico As Dictionary(Of String, jobLimitInfos), ByVal instanceId As Integer, ByVal res As Native.Api.Structs.QueryResult)
    Private Sub GotRefreshed(ByVal _dicoNew As List(Of String), ByVal _dicoDel As List(Of String), ByVal Dico As Dictionary(Of String, jobLimitInfos), ByVal instanceId As Integer, ByVal res As Native.Api.Structs.QueryResult)
