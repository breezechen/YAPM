...
Public Class mainNetworkConnectionsList
    Inherits customLV
    ...
    Public Overloads Sub Dispose()
        MyBase.Dispose()
        Me.ClearItems()
        RemoveHandler NetworkConnectionsProvider.GotRefreshed, AddressOf Me.GotRefreshed
...
#Region "Update methods & callbacks"
    ...
    Private Delegate Sub degGotNewItems(ByVal news As List(Of String), ByVal newItems As Dictionary(Of String, networkInfos))
    Private Sub GotNewItems(ByVal news As List(Of String), ByVal newItems As Dictionary(Of String, networkInfos))
        ...
        For Each id As String In news
            If _dico.ContainsKey(id) = False Then
                Dim netw As New cNetwork(newItems(id))
                netw.NewCount = 3
                _dico.Add(id, netw)
                ...
                For x As Integer = 1 To _subItems.Length - 1
                    _subItems(x) = ""
                Next
                AddItemWithStyle(id, netw).SubItems.AddRange(_subItems)
            End If
        Next
    ...
    Private Delegate Sub degGotRefreshed(ByVal _dicoNew As List(Of String), ByVal _dicoDel As List(Of String), ByVal Dico As Dictionary(Of String, networkInfos), ByVal instanceId As Integer, ByVal res As Native.Api.Structs.QueryResult)
    Private Sub GotRefreshed(ByVal _dicoNew As List(Of String), ByVal _dicoDel As List(Of String), ByVal Dico As Dictionary(Of String, networkInfos), ByVal instanceId As Integer, ByVal res As Native.Api.Structs.QueryResult)
